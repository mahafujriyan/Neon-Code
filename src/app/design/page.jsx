...
export default function DesignDashboard() {
  ...
  // আপনার রিকোয়ারমেন্ট অনুযায়ী ম্যানেজার লিস্ট
  ...
  const dailyFiltered = managerData.filter(d => 
    (d.submittedDate || d.createdAt?.split('T')[0]) === selectedDate
  );

  const monthlyFiltered = managerData.filter(d => 
    (d.submittedDate || d.createdAt)?.startsWith(currentMonth)
  );
  ...
  if (loading) return <div className="h-screen flex items-center justify-center font-black animate-pulse text-indigo-600 tracking-widest uppercase">Loading Design Data...</div>;
    ...
    <div className="min-h-screen p-4 md:p-8 bg-[#f8fafc] dark:bg-[#020617]">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
             <h1 className="text-xl font-black uppercase text-indigo-600 dark:text-indigo-400 leading-none">Design Performance</h1>
             <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">Manager: {filterManager}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
        <div className="bg-white dark:bg-[#0f172a] p-6 rounded-[2.5rem] shadow-sm border dark:border-slate-800">
            <h3 className="text-[14px] font-black uppercase text-slate-400 mb-4 ml-2 tracking-widest flex items-center gap-2">
                <span className="w-2 h-2 bg-indigo-500 rounded-full animate-ping"></span> Daily Summary: {filterManager}
            </h3>
        ...
        <div className="bg-white dark:bg-[#0f172a] p-6 rounded-[2.5rem] shadow-sm border dark:border-slate-800">
            <h3 className="text-[14px] font-black uppercase text-slate-400 mb-4 ml-2 tracking-widest flex items-center gap-2">
                <span className="w-2 h-2 bg-purple-500 rounded-full animate-pulse"></span> Monthly: {filterManager}
            </h3>
            <div className="grid grid-cols-5 gap-2">
